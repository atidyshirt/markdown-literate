...
Tangle.get_cursor_code_block = function(buffer)
  local code_block = {
    info_string = helpers.get_cursor_info_string(buffer, utils.get_node_at_cursor(0, true)),
    code_block = lines(helpers.get_cursor_code_block(buffer, utils.get_node_at_cursor(0, true))),
    language = helpers.process_language_from_info_string(
      helpers.get_cursor_info_string(buffer, utils.get_node_at_cursor(0, true))
    ),
...
Tangle.set_edit_buffer_options = function(edit_buffer, code, original_buffer, window_options)
  ...
  vim.api.nvim_command(string.format("autocmd! * <buffer=%s>", edit_buffer))
  vim.api.nvim_command(
    string.format(
      [[ autocmd BufWritePost <buffer=%s> :silent! execute 'lua vim.api.nvim_buf_set_lines(%s, %s, %s, true, vim.api.nvim_buf_get_lines(%s, 0, -1, true))' | :silent! bdelete ]],
      edit_buffer,
      original_buffer,
      code.start_line,
      code.end_line,
...
Tangle.remove_tangled_files = function(buffer)
  ...
  for _, code_block in pairs(all_code_blocks) do
    if code_block.filepath and code_block.filepath ~= code_block.language then
      ...
      helpers.remove_files(code_block.filepath)
    end
  end
end
...
Tangle.tangle_file = function(buffer)
  ...
  for _, code_block in pairs(all_code_blocks) do
    code_block.filepath = helpers.process_filepath(code_block.filepath)
    helpers.create_file(code_block.filepath)
